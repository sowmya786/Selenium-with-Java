...
public class Locators {
	...
	public static void main(String args[]) throws InterruptedException {
		...
		/*
		 * List<WebElement> Featurelinks = driver.findElements(By.
		 * xpath("//div[@class='container']/ul[@class='nav nav-tabs']/li"));
		 * System.out.println(Featurelinks.size());
		 * 
		 * for(WebElement links : Featurelinks) { System.out.println(links.getText());
		 * 
		 * try { links.click(); System.out.println("Clicked on the link");
		 * driver.navigate().back(); System.out.println("Navigated back");
		...
		/*
		 * // Step 3: Loop through each tab and click it for (int i = 0; i <
		 * featureLinks.size(); i++) { // Re-locate the tab list on each iteration to
		 * avoid StaleElementReferenceException featureLinks = driver.findElements(By.
		 * xpath("//div[@class='container']/ul[@class='nav nav-tabs']/li"));
		 * 
		 * WebElement tab = featureLinks.get(i); String tabText = tab.getText();
		 * System.out.println("Trying to click on: " + tabText);
		 * 
		 * try { tab.click(); System.out.println("Clicked on: " + tabText);
		 * Thread.sleep(1000); } catch (Exception e) {
		 * System.out.println("Could not click on: " + tabText);
		 * 
		 * }
		 * 
		 * finally { // Close the browser after the loop is done driver.quit(); } } } }
		 */
